...
class SingleFieldLinearNormalizer:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Stats are loaded from JSON on CPU while live inference tensors may
        # already be on the model GPU. Align the affine parameters per call so
        # normalization works for both CPU and CUDA clients.
