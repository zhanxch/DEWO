...
class EveManifestRobotVideoDataset(RobotVideoDataset):
        ...
        **kwargs,
        ...
        if self.unit_filter in {None, "", "all"}:
            ...
            raise ValueError(
                ...
                "'dewo_v9_pool', or 'dewo_scratch_pool', "
    ...
    def _passes_unit_filter(self, unit: dict[str, Any]) -> bool:
        ...
        if filt in {"dewo_v9_pool", "dewo_scratch_pool"}:
    ...
    def _v9_video_loss_weight(cls, unit: dict[str, Any], *, dplus_video_bc: bool = False) -> float:
        ...
        if unit.get("sample_type") == "episode" and role == "primary":
            return 1.0
        if dplus_video_bc and role == "primary":
            return 1.0
        return 0.0
    ...
    def _get_eve(self, idx: int) -> dict[str, Any]:
        ...
        if unit_filter in {"dewo_v9_pool", "dewo_scratch_pool"}:
            ...
            video_w = self._v9_video_loss_weight(
                unit, dplus_video_bc=unit_filter == "dewo_scratch_pool"
            )
        else:
