...
class FastWAM(torch.nn.Module):
    ...
    def infer_action(
        ...
        # w = text_cfg_scale in ε_cfg = ε_base + w (ε_posi − ε_base).
        # w=0 本体, w=1 纯优势, w>1 CFG guide.
        use_text_cfg = text_cfg_scale != 0.0
        ...
        if adaptive_tau is not None:
            ...
            if not use_text_cfg:
                raise ValueError(
                    "adaptive CFG requires a mix (`text_cfg_scale != 0`). "
        ...
        if use_text_cfg:
            if use_prompt and negative_prompt is None:
                raise ValueError(
                    "`text_cfg_scale != 0` with prompt input requires an explicit `negative_prompt` "
                    "(base / 本体 branch)."
                )
            if use_context and not use_negative_context:
                raise ValueError(
                    "`text_cfg_scale != 0` with cached context requires both "
                    "`negative_context` and `negative_context_mask` (base / 本体 branch)."
                )
        ...
        # S0 本体 = adapter off + base text (w=0). CFG posi = adapter on + success (w=1).
        ...
        for step_t_action, step_delta_action in zip(infer_timesteps_action, infer_deltas_action):
            ...
            if use_text_cfg:
                ...
                # Origin is always ε_base. mix_weight is text_cfg_scale (the formula w).
    ...
    def save_checkpoint(self, path, optimizer=None, step=None):
        ...
        value_state = export_value_head_state_dict(self)
        if value_state:
    ...
    def load_checkpoint(
        ...
        if value_state:
            if getattr(self, "value_head", None) is None:
                logger.warning(
                    "Checkpoint has value_head weights but the model has no value_head; skipping."
                )
            else:
                from fastwam.models.wan22.value_head import load_value_head_state_dict

                load_value_head_state_dict(self, value_state)
