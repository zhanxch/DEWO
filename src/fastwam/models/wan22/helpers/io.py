...
def hash_model_file(path, with_shape=True):
    ...
    # This digest is only a deterministic model-structure/cache key, never a
    # security primitive. Explicitly mark that intent for security linters.
    return hashlib.md5(keys_str, usedforsecurity=False).hexdigest()
