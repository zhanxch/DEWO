...
CFG inference is the mix formula. ``text_cfg_scale`` **is** the mix weight ``w``:

``ε_cfg = ε_base + w · (ε_posi − ε_base)``

- ``w=0``: 本体 / base (adapter off + base prompt). Skip the posi branch.
- ``w=1``: 纯优势 ``ε_posi`` (adapter on + success prompt).
- ``w>1``: CFG guide, e.g. ``w=2`` → ``ε_base + 2(ε_posi − ε_base)``.
...
def load_uncond_adapter_state_dict(
    ...
    if isinstance(state_dict_or_path, str):
        payload = torch.load(state_dict_or_path, map_location="cpu", weights_only=True)
...
def cfg_mix_is_active(text_cfg_scale: float) -> bool:
    """Two-branch mix iff ``w = text_cfg_scale`` is not 0."""
    return float(text_cfg_scale) != 0.0
...
def v5_infer_use_adapter(*, branch: str, use_text_cfg: bool) -> bool:
    ...
    - ``base``: always off (本体)
    - ``posi``: on iff the mix is active (``text_cfg_scale != 0``)
    """
    if branch == "base":
        return False
    if branch == "posi":
...
def v5_infer_remap_to_base_context(
    *,
    ...
    """``w=0`` uses the base prompt; remap the single branch onto it."""
...
def adaptive_cfg_mix_weight(
    *,
    ...
    ``guided_scale`` is the mix weight ``w`` (``text_cfg_scale``). ``w=0`` is
    本体 and has nothing to adapt; low-E fallback is still ``w=0``.
    """
    ...
    if not math.isfinite(scale) or scale == 0.0:
        raise ValueError(
            "`guided_scale` must be a non-zero mix weight (e.g. 1.0 or 2.0). "
            "text_cfg_scale=0 is 本体; low-E fallback is mix weight 0."
        )
