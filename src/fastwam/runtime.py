...
def create_fastwam_joint(
    ...
    from .models.wan22.uncond_adapter import normalize_uncond_adapter_config
    from .models.wan22.value_head import attach_recoverability_value_head
    ...
    model = FastWAMJoint.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
    ...
    model.fill_vae_latent_cache = bool(fill_cache)
    model.vae_latent_cache_dir = None if cache_dir is None else str(cache_dir)
    model.pin_video_context_to_base = False
    model.video_identity_lock_lambda = 0.0
    model.action_residual_lock_lambda = 0.0
    model.video_bc_on_zero_action = False
    if isinstance(uncond_adapter, DictConfig):
        uncond_adapter = OmegaConf.to_container(uncond_adapter, resolve=True)
    uncond_adapter_cfg = normalize_uncond_adapter_config(uncond_adapter)
    if bool(uncond_adapter_cfg.get("enabled", False)):
        raise ValueError(
            "DEWO(scratch) FastWAMJoint does not use `uncond_adapter`. "
            "Set `model.uncond_adapter=null` and attach `model.value_head` instead."
        )
    if isinstance(value_head, DictConfig):
        value_head = OmegaConf.to_container(value_head, resolve=True)
    if value_head is not None and bool(dict(value_head).get("enabled", False)):
        attach_recoverability_value_head(model, dict(value_head), recipe="v9")
    return model
