...
class PolicyIoCfgTest(unittest.TestCase):
    ...
    def test_partial_cached_context_is_rejected_even_with_prompt(self) -> None:
        image = {"input_image": np.zeros((3, 16, 16), dtype=np.float32)}
        with self.assertRaisesRegex(ValueError, "both 'context' and 'context_mask'"):
            validate_policy_observation({**image, "context": np.ones((2, 5)), "prompt": "task"})
        with self.assertRaisesRegex(ValueError, "both 'context' and 'context_mask'"):
            validate_policy_observation({**image, "context_mask": np.ones(2, dtype=bool), "prompt": "task"})
