...
class RecoverabilityFrontierTest(unittest.TestCase):
    ...
    def test_pass10_full_hit_then_zero_is_a_valid_pair(self) -> None:
        rows = [prefix(48, 10, pass_m=10), prefix(72, 0, pass_m=10)]
        found = frontier.find_recoverability_frontiers(rows, pass_m=10, max_steps=1200)
        self.assertEqual(len(found), 1)
        self.assertFalse(found[0]["prefix_is_mixed"])
        self.assertTrue(
            frontier.training_pair_eligible(
                {"seed_classification": "mixed"}, found[0], pass_m=10
            )
        )
...
class PrefixPassAtMTest(unittest.TestCase):
    ...
    def test_pass10_stops_only_after_all_trials(self) -> None:
        rows = [{"success": True}] * 9
        self.assertFalse(frontier.prefix_scan_should_stop(rows, pass_m=10))
        self.assertTrue(
            frontier.prefix_scan_should_stop(rows + [{"success": False}], pass_m=10)
        )
...
class RecoverabilityScanOrchEnvTest(unittest.TestCase):
    def test_shard_worker_env_pins_egl_to_the_same_gpu(self) -> None:
        sys.path.insert(0, str(ROOT / "scripts" / "fold_glasses"))
        import run_recoverability_pair_scan as orch

        env = orch.shard_worker_env("2")
        self.assertEqual(env["CUDA_VISIBLE_DEVICES"], "2")
        self.assertEqual(env["MUJOCO_EGL_DEVICE_ID"], "2")
        self.assertEqual(env["MUJOCO_GL"], "egl")
        self.assertEqual(env["CUDA_DEVICE_ORDER"], "PCI_BUS_ID")


class CopyFailureFullVideoTest(unittest.TestCase):
    def test_copy_video_file_writes_destination(self) -> None:
        import tempfile

        with tempfile.TemporaryDirectory() as tmp:
            src = Path(tmp) / "src.mp4"
            dst = Path(tmp) / "pair" / "failure_full_front.mp4"
            src.write_bytes(b"fake-mp4")
            frontier.copy_video_file(src, dst)
            self.assertTrue(dst.is_file())
            self.assertEqual(dst.read_bytes(), b"fake-mp4")
