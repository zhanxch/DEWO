...
class DewoV9UncondAdapterTests(unittest.TestCase):
    ...
    def test_text_cfg_scale_is_the_mix_weight(self) -> None:
        self.assertFalse(cfg_mix_is_active(0.0))
        self.assertTrue(cfg_mix_is_active(1.0))
        self.assertTrue(cfg_mix_is_active(2.0))
        self.assertTrue(
            v5_infer_remap_to_base_context(
        ...
        base = torch.zeros(2)
        posi = torch.ones(2)
        cfg0, _ = mix_guided_action_epsilon(base, posi, mix_weight=0.0)
        cfg1, _ = mix_guided_action_epsilon(base, posi, mix_weight=1.0)
        cfg2, _ = mix_guided_action_epsilon(base, posi, mix_weight=2.0)
        self.assertTrue(torch.equal(cfg0, base))
        self.assertTrue(torch.equal(cfg1, posi))
        self.assertTrue(torch.equal(cfg2, torch.full((2,), 2.0)))

    def test_adaptive_allows_weight_one_and_rejects_zero(self) -> None:
        self.assertEqual(
            adaptive_cfg_mix_weight(exec_rms=1.0, tau=0.05, guided_scale=1.0),
            1.0,
        )
        self.assertEqual(
            adaptive_cfg_mix_weight(exec_rms=0.0, tau=0.05, guided_scale=1.0),
            0.0,
        )
        with self.assertRaisesRegex(ValueError, "non-zero"):
            adaptive_cfg_mix_weight(exec_rms=1.0, tau=0.05, guided_scale=0.0)
