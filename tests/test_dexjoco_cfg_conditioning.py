...
from fastwam.inference import obs as obs_module
...
class DexJoCoCfgConditioningTest(unittest.TestCase):
    ...
    def test_prompt_mode_sends_explicit_success_and_base_prompts(self) -> None:
        adapter = DexJoCoFastWAMAdapter(self._settings(use_prompt=True))
    ...
    def test_cached_mode_loads_both_success_and_base_contexts(self) -> None:
        adapter = DexJoCoFastWAMAdapter(self._settings(use_prompt=False))
        ...
        with mock.patch.object(obs_module, "load_text_context_arrays", side_effect=fake_load) as loader:
            observation = adapter.env_obs_to_policy_obs(
                self._env_obs(),
                camera_key="front",
                camera_mapping={"base": "front"},
