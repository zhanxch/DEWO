...
class PolicyClient:
    ...
    def _init_socket(self) -> None:
        # Reconnects happen after timeouts; close the previous socket first so
        # repeated failures do not leak file descriptors and ZMQ resources.
...
def start_server_process(
    ...
    *,
    ...
    # Keep the descriptor open through Popen so the child inherits it, then
    # close the parent's handle. NamedTemporaryFile avoids the race in
    # tempfile.mktemp while preserving the path for diagnostics.
    with tempfile.NamedTemporaryFile(
        prefix="fastwam_server_stderr_", suffix=".log", mode="w", delete=False
    ) as stderr_fh:
