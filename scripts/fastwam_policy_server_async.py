...
class PolicyServerAsync:
    ...
    def _get_actions_batch(self, observations: list[dict], options: dict | None = None) -> dict:
        ...
        for observation in observations:
            ...
            # Policies in this repository historically returned either an
            # action dict or ``(action_dict, metadata)``. Normalize both forms
            # so the batch endpoint also works with the mock/legacy policy.
            if isinstance(result, (tuple, list)) and len(result) == 2:
                result = result[0]
            if not isinstance(result, dict) or "action" not in result:
                raise RuntimeError(
                    "Policy get_action must return an action dict or "
                    f"(action dict, metadata), got {type(result).__name__}"
                )
    ...
    def run(self) -> None:
        ...
        # A kill request sets ``running`` false inside a worker and queues its
        # response. Wait for that worker, then flush queued replies so clients
        # never time out waiting for the acknowledgement.
        ...
        while True:
            ...
            try:
                ...
                print(f"[policy-server-async] final reply send failed: {exc}", flush=True)
