...
try:
    import mujoco
except ModuleNotFoundError:  # Optional simulator dependency for utility users.
...
def snapshot_integration_state(env: Any) -> tuple[np.ndarray, dict[str, Any]]:
    if mujoco is None:
        raise RuntimeError("MuJoCo is required for integration-state snapshots")
    raw = env.unwrapped
...
def restore_integration_state(
    ...
    if mujoco is None:
        raise RuntimeError("MuJoCo is required for integration-state restoration")
    raw = env.unwrapped
    specification = mujoco.mjtState.mjSTATE_INTEGRATION
