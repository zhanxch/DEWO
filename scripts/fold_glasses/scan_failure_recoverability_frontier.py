...
records a full Pass@M (default 10) trials. The first 0/M prefix is the
...
Pass@M therefore captures RGB for every continuation frame and writes the
...
if (
    ...
    os.environ["MUJOCO_EGL_DEVICE_ID"] = _visible
...
def find_recoverability_frontiers(
    ...
    *,
    ...
    first 0/M unrecoverable scan point. The event is ``[M-33, M)``. A full
    Pass@M hit followed by 0/M is a valid pair. Isolated later recovery
    islands are ignored because the scanner stops at the first cliff.
    """
        ...
        return [
            {
                ...
                "prefix_is_mixed": bool(
                    1 <= int(last_hit["success_count"]) < (row_pass_m or pass_m or DEFAULT_PASS_M)
                ),
...
def training_pair_eligible(
    ...
    """Return whether the first 0/M cliff can be materialized as a pair."""
...
def estimate_scan_cost(
    *,
    ...
    return {
        ...
        "policy_load_count": 1,
        "note": (
            "Upper bound assumes every prefix runs all Pass@M trials to "
...
def copy_video_file(src: Path, dst: Path) -> None:
    """Copy a source rollout video into the pair directory without re-encode."""
...
def materialize_frontier_pair(
    ...
    *,
    ...
    for candidate in candidates:
        ...
        if not saved_success_rollout_videos_complete(candidate):
            ...
            print(
                ...
                f"success rollout RGB; trying next Pass@M candidate",
                flush=True,
            )
            continue
    ...
    failure_full_front_video = pair_dir / "failure_full_front.mp4"
    failure_full_wrist_video = pair_dir / "failure_full_wrist.mp4"
    ...
    copy_video_file(source_paths["front"], failure_full_front_video)
    copy_video_file(source_paths["wrist"], failure_full_wrist_video)
    ...
    failure_descriptor = {
        ...
        **common,
        ...
        "full_front_video": str(failure_full_front_video.resolve()),
        "full_wrist_video": str(failure_full_wrist_video.resolve()),
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    run_signature = {
        ...
        "text_embedding": (
            ...
            else str(args.text_embedding.expanduser().resolve())
        ),
        "task_name": str(args.task_name),
    ...
    text_emb = args.text_embedding
    if text_emb is not None:
        text_emb = text_emb.expanduser().resolve()
