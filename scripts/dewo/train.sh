...
if [[ -n "${FITWAM_ENV_PREFIX:-}" && "${FITWAM_ENV_PREFIX}" == "${FITWAM_CONDA_ROOT}/envs/"* && -x "${FITWAM_ENV_PREFIX}/bin/python" ]]; then
...
if [[ "${DEWO_INIT:-}" == "scratch" ]]; then
  echo "[dewo] init=${INIT_WEIGHTS} (hash only; resume=null)"
  RESUME_OVERRIDE="resume=null"
else
  echo "[dewo] init=${FASTWAM_RESUME}"
  RESUME_OVERRIDE="resume=${FASTWAM_RESUME}"
fi
