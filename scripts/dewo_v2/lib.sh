...
#
# DEWO_VERSION=v9 INIT=s0: frozen mixed-S0 MoT + text-side K/V adapter.
# INIT=scratch: full FastWAMJoint from Wan + ActionDiT (not frozen S0).
# Mix ε_cfg = ε_0 + g w (ε_+ − ε_0).
...
dewo_v2_assert_not_lora() {
  ...
  if [[ "${value}" == *lora* || "${value}" == *LoRA* ]]; then
    ...
    echo "  LoRA recipes are removed. Use INIT=s0 (adapter) or INIT=scratch (joint)." >&2
    echo "  Hydra: dexjoco/dexjoco_dewo_v9_offline_b1_jump_fast_uncond (INIT=s0)" >&2
    echo "         dexjoco/dexjoco_dewo_scratch_joint (INIT=scratch)" >&2
    return 2
  fi
}
...
dewo_v2_load_task() {
  ...
  if ! eval "$(python "${root}/scripts/dewo_v2/tasks.py" export-env --task "${task}")"; then
    echo "[dewo-v2] ERROR: tasks.py export-env failed for TASK=${task}" >&2
    return 2
  fi
...
# In-repo mixed FastWAMJoint 55K + 224/z-score eval stack. Do not require
# FastWAM-infer-in-DexJoco or per-task T5 .pt files.
dewo_v2_joint_s0_defaults() {
  local root="${ROOT_DIR:?ROOT_DIR must be set before sourcing lib.sh}"
  JOINT_S0_CKPT="${JOINT_S0_CKPT:-${root}/checkpoints/dexjoco/mixed_5task_fastwam_joint/weights/step_055000.pt}"
  JOINT_S0_STATS="${JOINT_S0_STATS:-${root}/artifacts/mixed_5task/dataset_stats.json}"
  JOINT_S0_CFG="${JOINT_S0_CFG:-${root}/configs/eval/dexjoco/mixed_5task_fastwam_joint/config.yaml}"
  ACTION_DIT="${ACTION_DIT:-${root}/checkpoints/ActionDiT_linear_interp_Wan22_alphascale_1024hdim.pt}"
  export JOINT_S0_CKPT JOINT_S0_STATS JOINT_S0_CFG ACTION_DIT
}

dewo_v2_pin_joint_s0() {
  dewo_v2_joint_s0_defaults
  export CKPT="${JOINT_S0_CKPT}"
  export STATS="${JOINT_S0_STATS}"
  export PRETRAINED_NORM_STATS="${JOINT_S0_STATS}"
  export INIT_WEIGHTS="${JOINT_S0_CKPT}"
  export SOURCE_CHECKPOINT="${JOINT_S0_CKPT}"
  export SOURCE_CONFIG="${JOINT_S0_CFG}"
  export FASTWAM_SOURCE_CONFIG="${JOINT_S0_CFG}"
  export MODEL_CONFIG="${JOINT_S0_CFG}"
}

dewo_v2_resolve_open_repo() {
  local root="${ROOT_DIR:?ROOT_DIR must be set before sourcing lib.sh}"
  local candidate="${OPEN_REPO:-${root}/../FastWAM-infer-in-DexJoco}"
  if [[ -d "${candidate}" ]]; then
    OPEN_REPO="$(cd "${candidate}" && pwd)"
  else
    OPEN_REPO="${candidate}"
    echo "[dewo-v2] OPEN_REPO missing (${candidate}); using in-repo joint eval stack" >&2
  fi
  export OPEN_REPO
}

dewo_v2_activate_fastwam() {
  local root="${ROOT_DIR:?ROOT_DIR must be set before activating an environment}"
  # shellcheck disable=SC1091
  source "${root}/scripts/fitwam_env.sh"
...
dewo_v2_align_opensource_stack() {
  ...
  local mixed_stats="${root}/artifacts/mixed_5task/dataset_stats.json"
  local local_cfg="${root}/configs/eval/dexjoco/mixed_5task_fastwam_joint/config.yaml"

  if [[ -f "${local_cfg}" ]]; then
    SOURCE_CONFIG="${SOURCE_CONFIG:-${local_cfg}}"
  elif [[ -f "${open_cfg}" ]]; then
  ...
  if [[ -z "${stats}" || ! -f "${stats}" || "${stats}" == */meta/* || "${stats}" == *"${task}_fastwam"* ]]; then
    if [[ -f "${mixed_stats}" ]]; then
      stats="${mixed_stats}"
    elif [[ -f "${open_stats}" ]]; then
      stats="${open_stats}"
    fi
  fi
  STATS="${stats}"
