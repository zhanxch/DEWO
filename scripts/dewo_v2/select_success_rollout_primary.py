...
from dewo_v2.tasks import is_env_success, is_train_success, train_success_cap
...
def is_complete_success(
    outcome: dict[str, Any],
    length: int,
    *,
    task_name: str | None = None,
) -> bool:
    """Train-eligible complete success (D0 / leftover val).

    hammer_nail env successes longer than the train cap stay successes, but
    they are not complete *training* successes.
    """

    if int(length) <= MIN_COMPLETE_LENGTH:
        return False
    return is_train_success(outcome, length, task_name=task_name)


def is_env_complete_success(outcome: dict[str, Any], length: int) -> bool:
    return is_env_success(outcome) and int(length) > MIN_COMPLETE_LENGTH
...
def select_primary(
    *,
    ...
    for row in outcomes:
        ...
        if is_complete_success(row, length, task_name=task_name):
...
def select_one_per_all_success_seed(
    *,
    ...
    """D0 = one train-eligible success per 4/4 *env*-success seed.

    4/4 is environment success (long hammer_nail wins still count). D0 is
    picked only from train-eligible episodes (hammer_nail length ≤ cap).
    Over-cap successes go to the excluded/test bucket, not train/val.
    """
    ...
    for seed in sorted(by_seed):
        rows = by_seed[seed]
        env_ok = [r for r in rows if is_env_complete_success(r, int(r["length"]))]
        train_ok = [
            r
            for r in rows
            if is_complete_success(r, int(r["length"]), task_name=task_name)
        ]
        over_cap = [
            r
            for r in env_ok
            if int(r["episode_index"]) not in {int(x["episode_index"]) for x in train_ok}
        ]
        fails = [r for r in rows if not is_env_complete_success(r, int(r["length"]))]
        excluded.extend(fails)
        excluded.extend(over_cap)
        if env_ok and len(env_ok) == len(rows):
            if train_ok:
                ordered = sorted(
                    train_ok,
                    key=lambda r: (
            ...
            rows.append(
                {
                    ...
                    "episode_outcome": (
                        "success" if is_env_success(row) else "failure"
                    ),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Task id for prepare-only train-success length cap "
            "(hammer_nail: expert_max + slack). Env success is unchanged."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    task_name = str(args.task_name).strip() if args.task_name else None
    if args.mode == "one_per_all_success_seed":
        primary, leftover, failures = select_one_per_all_success_seed(
            outcomes=outcomes, lengths=lengths, task_name=task_name
        )
    ...
    payload = {
        ...
        "task_name": task_name,
        "train_success_cap": train_success_cap(task_name) if task_name else None,
