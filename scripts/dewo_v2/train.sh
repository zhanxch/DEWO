...
#
# Scratch (full FastWAMJoint, not frozen S0):
...
CALLER_INIT_WEIGHTS="${INIT_WEIGHTS:-}"
...
case "${INIT}" in
  ...
  scratch)
    ...
    export DEWO_INIT=scratch
    export DEWO_TASK="dexjoco/dexjoco_dewo_scratch_joint"
    export DEWO_VARIANT="${CALLER_DEWO_VARIANT:-DEWO-scratch-joint}"
    export DEWO_PROTOCOL="${CALLER_DEWO_PROTOCOL:-${TASK}_dewo_scratch_joint}"
    export TMUX_SESSION="${TMUX_SESSION:-${TASK}_dewo_scratch_joint}"
    dewo_v2_joint_s0_defaults
    export INIT_WEIGHTS="${CALLER_INIT_WEIGHTS:-${ACTION_DIT}}"
    export DEWO_SOURCE_SHA="${DEWO_SOURCE_SHA:-scratch}"
    ;;
  lora)
    echo "[dewo-train] ERROR: LoRA recipes are removed. Use INIT=s0 or INIT=scratch." >&2
    exit 2
    ;;
  *)
    echo "[dewo-train] ERROR: INIT must be s0 or scratch, got ${INIT}" >&2
    exit 2
    ;;
esac
...
if [[ "${INIT}" == "scratch" ]]; then
  export DEWO_OUTPUT_DIR="${CALLER_OUTPUT_DIR:-./runs/dexjoco_${TASK}_dewo_scratch_joint}"
  export FITWAM_WANDB_GROUP="${CALLER_WANDB_GROUP:-${TASK}_dewo_scratch_joint}"
  export RUN_ID="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_${DEWO_VARIANT}}"
  export WANDB_RUN_NAME="${WANDB_RUN_NAME:-${TASK}_dewo_scratch_${RUN_ID}}"
else
...
echo "[dewo-train] TASK=${TASK} INIT=${INIT} DEWO_INIT=${DEWO_INIT:-} DEWO_VERSION=${DEWO_VERSION} GPUS=${GPUS}"
echo "[dewo-train] DEWO_TASK=${DEWO_TASK}"
echo "[dewo-train] INIT_WEIGHTS=${INIT_WEIGHTS}"
