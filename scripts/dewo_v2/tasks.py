...
DEFAULT_NFE = 10
# hammer_nail can keep retrying until the nail goes in, so policy successes
# can dwarf expert length. Prepare D0 / train-success uses expert_max + slack.
# Collect and official 4×50 eval do not apply this cap; env success stays success.
COLLECT_CAP_TO_EXPERT_PLUS = frozenset({"hammer_nail"})
COLLECT_EXPERT_SLACK_FRAMES = 10
...
def resolve_ckpt(task: TaskSpec, *, root: Path = ROOT, open_repo: Path = DEFAULT_OPEN_REPO) -> Path:
    override = str(os.environ.get("CKPT", "")).strip()
    if override:
    ...
    if opened.is_file():
        return opened
    mixed_joint = (
        root / "checkpoints/dexjoco/mixed_5task_fastwam_joint/weights/step_055000.pt"
    ).resolve()
    if mixed_joint.is_file():
        return mixed_joint
    raise FileNotFoundError(
        f"Missing checkpoint for {task.name}: tried {local}, {opened}, and {mixed_joint}"
    )
...
def expert_episode_lengths(task: TaskSpec, *, root: Path = ROOT) -> list[int]:
    """Frame counts from the expert LeRobot `meta/episodes.jsonl`."""

    path = resolve_expert(task, root=root) / "meta" / "episodes.jsonl"
    if not path.is_file():
        raise FileNotFoundError(f"Missing expert episodes.jsonl: {path}")
    lengths: list[int] = []
    for line in path.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        n = row.get("length") or row.get("episode_length") or row.get("num_frames")
        if n is None:
            continue
        lengths.append(int(n))
    if not lengths:
        raise ValueError(f"No episode lengths in {path}")
    return lengths


def resolve_collect_max_steps(task: TaskSpec, *, root: Path = ROOT) -> int:
    """Numeric cap used for hammer_nail prepare/train-success length.

    Official 4×50 eval keeps ``task.max_steps`` (1000). Collect records the
    env outcome as-is. Prepare D0 excludes hammer_nail successes longer than
    expert_max + slack; it does not relabel them as failures.
    """

    base = int(task.max_steps)
    if task.name not in COLLECT_CAP_TO_EXPERT_PLUS:
        return base
    return max(expert_episode_lengths(task, root=root)) + int(
        COLLECT_EXPERT_SLACK_FRAMES
    )


def train_success_cap(task_name: str, *, root: Path = ROOT) -> int | None:
    """Max length for *training* success data, or None if uncapped.

    hammer_nail only, applied at prepare. A long env success is still a
    success; it just does not enter D0 / leftover val.
    """

    name = str(task_name).strip()
    if name not in COLLECT_CAP_TO_EXPERT_PLUS:
        return None
    return resolve_collect_max_steps(get_task(name), root=root)


def is_env_success(outcome: Mapping[str, Any] | bool) -> bool:
    if isinstance(outcome, bool):
        return outcome
    if "success" in outcome:
        return bool(outcome["success"])
    return str(outcome.get("outcome", "")).lower() == "success"


def is_train_success(
    outcome: Mapping[str, Any] | bool,
    length: int,
    *,
    ...
    """Whether a rollout may enter D0 / train success data.

    Env success stays success even when this returns False.
    """

    if not is_env_success(outcome):
        return False
    cap_value = cap
    if cap_value is None and task_name:
        cap_value = train_success_cap(task_name, root=root)
    if cap_value is None:
        return True
    return int(length) <= int(cap_value)
...
def build_exports(
    ...
    *,
    ...
    mixed_stats = (root / "artifacts/mixed_5task/dataset_stats.json").resolve()
    if not stats.is_file() and mixed_stats.is_file():
        stats = mixed_stats
    ...
    return {
        ...
        "COLLECT_MAX_STEPS": resolve_collect_max_steps(task, root=root),
