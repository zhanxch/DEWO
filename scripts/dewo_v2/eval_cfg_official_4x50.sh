#!/usr/bin/env bash
# Official 4×50 DEWO v9 CFG eval, or mixed-S0 baseline with CFG_SCALE=0.
# Opensource 224 / z-score. In-repo async stack (no FastWAM-infer-in-DexJoco).
#   TASK=fold_glasses RUN_DIR=... CKPT=... GPUS=4,5,6,7 CFG_SCALE=2.0 \
#     CFG_GATE_MODE=value_growth bash scripts/dewo_v2/eval_cfg_official_4x50.sh
# Mixed-S0 joint baseline (本体, live T5):
#   TASK=fold_glasses GPUS=0,1,2,3 CFG_SCALE=0 LOAD_TEXT_ENCODER=1 \
#     RUN_DIR=configs/eval/dexjoco/mixed_5task_fastwam_joint \
#     CKPT=checkpoints/dexjoco/mixed_5task_fastwam_joint/weights/step_055000.pt \
#     WAIT_IDLE=0 bash scripts/dewo_v2/eval_cfg_official_4x50.sh
#
# CFG mix: ε_cfg = ε_base + w (ε_posi − ε_base), w=text_cfg_scale / CFG_SCALE.
...
# Mixed-S0 z-score; per-task OPEN stats mismatch collapses poses at infer.
if [[ -z "${CALLER_STATS}" ]]; then
  _mixed_probe="${BACKBONE_CKPT:-}${CKPT:-}"
  if [[ "${_mixed_probe}" == *mixed_5task* ]]; then
...
LOAD_TEXT_ENCODER="${LOAD_TEXT_ENCODER:-0}"
if [[ "${LOAD_TEXT_ENCODER}" == "1" ]]; then
  TEXT_EMBEDDING_CACHE_DIR="${TEXT_EMBEDDING_CACHE_DIR:-}"
else
  TEXT_EMBEDDING_CACHE_DIR="${TEXT_EMBEDDING_CACHE_DIR:?Set TEXT_EMBEDDING_CACHE_DIR (or LOAD_TEXT_ENCODER=1)}"
fi
...
if [[ "${LOAD_TEXT_ENCODER}" != "1" ]]; then
  required_paths+=("${TEXT_EMBEDDING_CACHE_DIR}")
fi
...
log "CFG mix: w=${CFG_SCALE} in ε_base + w(ε_posi-ε_base) (0=本体, 1=优势, >1=guide)."
...
import sys
sys.exit(0 if abs(float(sys.argv[1])) >= 1e-12 else 1)
PY
  then
    log "ERROR: adaptive CFG needs CFG_SCALE!=0 (mix); scale=0 is 本体"
    exit 2
  fi
fi
...
eval_one() {
  ...
  local text_args=()
  if [[ "${LOAD_TEXT_ENCODER}" == "1" ]]; then
    text_args=(--load-text-encoder)
  else
    text_args=(--no-load-text-encoder --text-embedding-cache-dir "${TEXT_EMBEDDING_CACHE_DIR}")
  fi
  if [[ -n "${NOISE_SEED_BASE}" ]]; then
  ...
  "${ENV_PREFIX}/bin/python" scripts/dexjoco_async/run_multi_gpu_dexjoco_eval.py \
    ...
    --client-conda-env "${CLIENT_CONDA_ENV:-dexjoco}" \
    --run-dir "${RUN_DIR}" \
    --checkpoint "${CKPT}" \
