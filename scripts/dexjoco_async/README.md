# DexJoCo Closed-Loop Eval / Collect (async ZMQ)

Policy loading, CFG/value inference, and DexJoCo obs conversion live in
``src/fastwam/inference``. This directory is the **async ZMQ** eval/collect
tooling (overlap control, LPF, multi-GPU servers).

For in-process eval (no ZMQ), use ``scripts/infer_dexjoco.py`` and
``scripts/eval_dexjoco.py``.
...
DexJoCo / DEWO v9 官方 in-process 评测用 `scripts/eval_dexjoco.py`（三种架构 + CFG/value）。本目录是 async 闭环工具。
