...
dexjoco_opensource_setup() {
  ...
  if [[ ! -d "${OPEN_REPO}" ]]; then
    echo "[opensource-eval] ERROR: OPEN_REPO not found: ${OPEN_REPO}" >&2
    echo "  Mixed/joint baseline eval does not use this stack." >&2
    echo "  Use: GPUS=0,1,2,3 bash scripts/dexjoco/eval_mixed_5task_opensource_4x50.sh" >&2
    return 1
  fi
