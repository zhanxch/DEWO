...
def summarize_job(job_dir: Path) -> dict | None:
    cfg_agg = job_dir / "aggregate.json"
    if cfg_agg.is_file():
        payload = json.loads(cfg_agg.read_text())
        runs = payload.get("runs") or []
        return {
            "job_dir": str(job_dir),
            "job_id": job_dir.name,
            "pooled": {
                "successes": payload.get("pooled_successes"),
                "episodes": payload.get("pooled_episodes"),
                "success_rate": payload.get("pooled_success_rate"),
            },
            "runs": runs,
            "mean_success_rate": payload.get("mean_success_rate"),
            "std_success_rate": payload.get("std_success_rate"),
            "var_success_rate": payload.get("var_success_rate"),
            "n_runs": len(runs),
        }
